...
/* brexx 28 JUL 19 Add BREXX Instruction Count statistics          */
...
numeric digits 14
if ^datatype(count,"N") then count=100        /* Repetition count */
if ^datatype(averaging,"N") then averaging=100    /* Averaging-over count */
...
say '----- REXXCPS' format(rexxcps,1,1) '-- Measuring REXX clauses/second -----'
...
elapsed=elapsed()-elp
say 'BREXX Instruction Count   'instrCount
say '      Elapsed Time [secs] 'format(elapsed/100,,2)
say '      Instructions/secs   'format(100*instrCount/elapsed,,2)
exit
/* Calculate current time in hundreds of a second */
elapsed:
  parse value time('L') with hh':'mm':'ss'.'hs
