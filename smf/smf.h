...
typedef struct smf242_start_record {       //                                 offset/len/   offset+1
    // base header
    short int      reclen;                 // Record Length                   0      2      1
    short int      segdesc;                // Segment Descriptor (RDW) -- 0   2      2      3
    unsigned char  sysiflags;              // System indicator flags          4      1      5
    ...
    unsigned char  ssi[4];                 // Subsystem identification        18     4     19
    short int      subrectype;             // Record subtype                  22     2     23
    // brexx header
    unsigned char  user[8] ;               // Current user identification     24     8     25
    unsigned char  runid[4];               // Kind of session identification  32     4     33
    // data
    unsigned char  dsname[54];             // dataset name                    36    54     37
    unsigned char  args[80];               // dataset name                    90    80     91

} SMF_242_START_RECORD, *P_SMF_242_START_RECORD;

typedef struct smf242_load_record {        //                                 offset/len/   offset+1
    // base header
    short int      reclen;                 // Record Length                   0      2      1
    short int      segdesc;                // Segment Descriptor (RDW) -- 0   2      2      3
    unsigned char  sysiflags;              // System indicator flags          4      1      5
...
typedef struct smf242_term_record {        //                                 offset/len/   offset+1
    // base header
    short int      reclen;                 // Record Length                   0      2      1
    short int      segdesc;                // Segment Descriptor (RDW) -- 0   2      2      3
