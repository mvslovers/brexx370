...
#include <setjmp.h>
/*
 * The following macros allow the internal CALL to an internal sub-procedure within the same procedure
 *   step 1 Define UseSubProc()  in the definition part of your procedure
 *   step 2 SCALL(your-sub-procedure)
 *   step 3 SCPROC(your-sub-procedure)   it is actually the label and some administrative stuff
 *   step 4 SRETURN from your sub-procedure
 *   This allows you to break up large structures in sub-modules and make it more readable
 *   there is nesting up to 4 levels allowed, meaning you can SCALL from within an sub-proc
 *   ** important the scope of the variables remain intact.
*/


#define UseSubPROC() jmp_buf __rlabel[3]; int __jumped[3]; int __level=0;
#define SCALL(c) setjmp(__rlabel[__level]); if (__jumped[__level]!=1) {__jumped[__level]=1;goto c;} __jumped[__level]=0;

#define SPROC(c) c:__level++;
#define SRETURN __level--; longjmp(__rlabel[__level], 1);
 ...
 */
    ...
    if (input_date == NULL) {
        ...
        SCALL(checkInputFormat);    // check and process certain input formats for input formats
...
//* Translate into appropriate date according to out format
    if      (strncasecmp(LSTR(*datestr), "XEUROPEAN",2) == 0) sprintf((char *) LSTR(*datestr), "%02d/%02d/%04d", parm[1], parm[2]
    else if (strncasecmp(LSTR(*datestr), "EUROPEAN",1) == 0)  sprintf((char *) LSTR(*datestr), "%02d/%02d/%02d", parm[1], parm[2]
    else if (strncasecmp(LSTR(*datestr), "DEC",3) == 0)       sprintf((char *) LSTR(*datestr), "%02d-%02s-%02d", parm[1], monthsS
    else if (strncasecmp(LSTR(*datestr), "XDEC",3) == 0)      sprintf((char *) LSTR(*datestr), "%02d-%02s-%04d", parm[1], monthsS
    else if (strncasecmp(LSTR(*datestr), "GERMAN",1) == 0)    sprintf((char *) LSTR(*datestr), "%02d.%02d.%02d", parm[1], parm[2]
    else if (strncasecmp(LSTR(*datestr), "USA",1) == 0)       sprintf((char *) LSTR(*datestr), "%02d/%02d/%02d", parm[2], parm[1]
    else if (strncasecmp(LSTR(*datestr), "XUSA",2) == 0)      sprintf((char *) LSTR(*datestr), "%02d/%02d/%02d", parm[2], parm[1]
    else if (strncasecmp(LSTR(*datestr), "ORDERED",1) == 0)   sprintf((char *) LSTR(*datestr), "%04d/%02d/%02d", parm[3], parm[2]
    else if (strncasecmp(LSTR(*datestr), "LONG",2) == 0)      sprintf((char *) LSTR(*datestr), "%02d %s %04d", parm[1], months[pa
    else if (strncasecmp(LSTR(*datestr), "SHORT",2) == 0)     sprintf((char *) LSTR(*datestr), "%02d %s %04d", parm[1], monthsSH[
    else if (strncasecmp(LSTR(*datestr), "STANDARD",1) == 0)  sprintf((char *) LSTR(*datestr), "%04d%02d%02d", parm[3], parm[2],
    else if (strncasecmp(LSTR(*datestr), "NORMAL",1) == 0)    sprintf((char *) LSTR(*datestr), "%02d %s %04d", parm[1], monthsSH[
    else if (strncasecmp(LSTR(*datestr), "WEEK",1) == 0)      STRCPY((char *)  LSTR(*datestr), WeekDays[(JDN + 1) % 7]);
    else if (strncasecmp(LSTR(*datestr), "JULIAN",1) == 0)    sprintf((char *) LSTR(*datestr), "%04d%03d", parm[3], JDN + 1 - JUL
