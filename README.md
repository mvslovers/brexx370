...
Brexx was originally written by Vasilis this version of REXX because he though that it would be nice to have his own REXX interpreter do use it with DOS, UNIX. This REXX interpreter is  written in ANSI C, and was designed to make it fast and flexible, with very little restrictions, without lossing compatibility with the Ansi-Rexx. It was edited and compiled for MVS (i.e. MVS 3.8j or tk4-) by Peter Jacob and Mike Großmann.
...
:warning: Make sure you change the line `//XMITIN   DD DSN=HERC01.BREXX.version.XMIT,DISP=SHR` to match your version of the release.
...
//SYSUT2   DD DSN=BREXX.V2R3M0.INSTALL,
//         UNIT=3390,
//         SPACE=(TRK,(300,60,20)),
...
1. A copy of JCC. A current version the JCC compile is available here: https://github.com/mvslovers/jcc . JCC is a precompiled C compiler for MVS.
...
:warning: this also changes the sockdev port to 3506

:warning: to change it back run the same same command as above but replace `ebcdic` with `ascii`.
...
| `TK4PORT     := 3506`                          | port of EBCDIC listener                                                                                                                                                                                                                    |
